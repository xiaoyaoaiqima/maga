...
输入 plan.selling_painpoint_expression 是本篇已确认的原始卖点痛点事实。认证、配方、数字、牧场制度等硬产品属性，要么来自本篇原始表达，要么属于下列已联网确认的莼悦基础事实；宝宝睡眠、便便、肠胃、自护力、吸收、长肉等 UGC 使用效果不受原始表达字面范围限制。允许自然转述、拆开表达，也允许穿插渠道、时间、地点、动作和心情。

必须遵守以下人工校准边界：
1. 只拦新增或改变的硬产品事实，包括认证、成分名称或数量、牧场制度、数字、周期和具体作用机制。正文新增“奶源知根知底”但原始表达只写“自家牧场”，属于 product_fact_expansion，判 hard / hold_out。
...
2. 已联网确认的莼悦基础事实可以跨本篇原始表达自然出现：莼悦含 5 重有机 HMO 和有机 GOS、有机活性 OPN、天然含有有机乳源 OPO 类似结构脂；公开发布信息还将 HMO/GOS 承接肚肚内环境、OPN 承接肚肚屏障、OPO 类似结构脂承接钙和关键脂肪酸吸收及大便柔软。不要把 HMO、OPN 本身判成新增错误配方。
3. 欧盟有机畜牧规则禁止使用激素和生长促进剂，但个体动物的兽医治疗存在例外。可以写“有机牧场不使用人工促生长/促产激素”；不要升级成“任何情况下绝不使用激素”。“奶牛更健康、奶源更安全”可按原始卖点作强种草转述，或写成消费者更放心、更安心；不得扩成已证明优于其他奶源、绝对安全或零风险。
4. 宝宝使用效果可以自然外扩并形成完整效果链，包括睡眠、便便、肠胃舒服、自护力、吸收、长肉、精神头、食欲和适应情况。不能仅因“牧场/纯天然之外新增睡眠、便便或肠胃效果”，也不能仅因“换季、自护力、吸收和长肉连成完整效果链”判 rewrite 或 hold_out；这类表达属于允许的强种草 UGC。只有写成治疗疾病、绝对保证，或新增精确可核验数据时才按实际问题拦截。
5. 抽象、不可量化的使用感受可以自然外扩，例如“现在喝着挺安稳”“宝宝喝得挺顺口”“我也放心了”。不能仅因这些话不在原始表达中判事实外扩。
   - 抽象感受指人的心情、笼统体验或不可核验评价；“奶源知根知底”描述的是产品或奶源属性，不属于抽象感受。
6. UGC 本来就是种草文。标题允许在原始事实基础上强化主观效果判断，例如“焦虑终结者”“让敏敏宝宝安心”；不能仅因标题强种草、带效果感就判 rewrite 或 hold_out。
7. 不强制文章完成“最终选择/购买莼悦”的闭环。只要自然建立了与莼悦的产品关系，停在被推荐、了解到、记下来或继续关注都可以；不能因没有下单或明确选择动作判问题。
8. 原始表达中的强因果、产品事实和效果已经业务确认，不主动降调，不因为表达力度强就判风险。
9. 像“就靠这个依据确认的”这种直接复述任务字段、审核字段或内容方向的话，属于 brief_translation_tone；事实和种草内核成立时判 minor / light_fix_usable，不要整篇否定。
10. “别的品牌介绍里从没这样说过”这类不点名竞品、不新增具体认证、成分或效果的自然比较感受允许 direct_pool。只有点名竞品并新增具体可核验对比事实，或出现明确贬损时，才按实际问题拦截。
11. 如果把产品事实写成罐身、包装、标签、说明或配料表上的文字，或发生主体归因错误、价格负向、医疗治疗、绝对保证，按实际严重程度判 rewrite/hard。
