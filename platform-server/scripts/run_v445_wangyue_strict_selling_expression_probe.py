...
EXPERIMENT_ID = "v445_strict_selling_expression_probe"
...
def _local_issues(title: str, body: str) -> list[str]:
    ...
    if any(term in text for term in ("可能", "也许", "不知道是不是", "功劳")):
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v445 strict-selling-expression probe preview",
        "",
        ...
        "本版只接回已有卖点表达槽位，不新增 Product Reason；相对 v444，进一步过滤防护比喻、固定杯数、强KOL词和免责声明式表达。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证旧卖点表达槽位接回这条核心链路后的效果。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v445 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v445`",
