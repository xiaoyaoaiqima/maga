...
EXPERIMENT_ID = "v442_single_chain_diary_probe"
...
def main() -> None:
    ...
    seeds = [_sample_seed(index) for index in range(1, args.count + 1)]
...
def _sample_seed(index: int) -> dict[str, str]:
    return {
        "item_no": str(index),
        ...
        "product_reason": random.choice(SEEDS["product_reason"]),
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v442 single-chain diary probe preview",
        "",
        ...
        "",
        "这批只验证一条链路：妈妈看到孩子喝旺玥的小现场，回想以前状态，再写现在变化，最后正面推荐。",
        "这不是最终架构；如果这条链路能跑出可读内容，下一步只从重复位置反抽槽位，把已有语料填进这些可替换点。",
        "",
...
def _candidate_slots() -> list[str]:
    return [
        ...
        "product_reason：旺玥被推荐的理由，成分/营养配置/接受度/3岁后日常营养",
        "mother_emotion：妈妈为什么想发这一条，意外、回头看、顺手记、觉得买对了",
        "ending_shape：推荐别人试试、自己继续买、停在生活现场、短句认可",
        "title_shape：短动作、状态结果、轻推荐、日记口吻、emoji少量扰动",
    ]
