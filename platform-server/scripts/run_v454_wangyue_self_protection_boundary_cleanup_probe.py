...
EXPERIMENT_ID = "v454_self_protection_boundary_cleanup_probe"
...
SEEDS = {
    ...
    "state_observation": [
        ...
        "以前一到傍晚就没精神，现在晚饭后还能自己找点事做",
        "出去一趟回来没有马上躺着，洗完手还想再玩一会儿",
        "活动量大一点也撑得住，后半天没有那么容易掉线",
        "晚上陪到挺晚，状态还在线，不像以前动一会儿就喊累",
        "回家路上没有一直喊累，到家后还愿意自己玩一阵",
        "外出回来小状态稳了很多，不是那种一下子蔫下去的感觉",
        "下午那股劲比以前足，回家后还能跟着家里节奏走",
        "一天下来精神头没有掉得那么快，妈妈肉眼能看出来",
        "出门玩完还有余量，回家后不需要一直催着哄着",
        "晚一点也没明显蔫下去，整个人看着更有劲",
        "回到家还愿意跟着大人收拾东西，不是直接瘫着",
        "下午活动完还有点余量，晚上节奏也没乱",
        "出去玩完还能自己安排一会儿，不用我一直盯着",
        "到家后小脸状态挺好，不像以前那样马上没电",
        "后半天还能跟上家里的节奏，看着没那么疲",
        "玩到晚一点也没有明显掉线，整个人还是亮亮的",
        "回家后愿意自己找玩具玩一会儿，不是一直喊累",
        "活动回来还能坐下来吃点东西，状态比以前稳",
        "傍晚那会儿没有突然蔫下去，妈妈看着很明显",
        "一整天下来状态更均匀，不是前面疯玩后面没电",
    ],
}
...
def main() -> None:
    ...
    response = {
        ...
        "model": config.model,
        "core_chain": "short inline core brief: diary trigger + prior weak state + varied Wangyue product-reason phrase + child-state observation + recommendation",
...
def _build_product_reason_clause(selling_expression_inline: str) -> str:
    shapes = [
        f"当时选旺玥，就是冲着{selling_expression_inline}来的",
        f"选旺玥时，我记住的是{selling_expression_inline}",
        f"旺玥这罐吸引我的，是{selling_expression_inline}",
        f"那会儿挑来挑去，最后选了旺玥，是因为{selling_expression_inline}",
        f"家里后来固定喝旺玥，跟{selling_expression_inline}有关系",
        f"我当时记住旺玥，就是因为{selling_expression_inline}",
...
def _build_core_content(seed: dict[str, str]) -> str:
    shapes = [
        (
            ...
            f"以前{_strip_leading_yiqian(seed['past_state'])}；{seed['product_reason_clause']}。"
            f"现在{seed['state_observation']}。{seed['recommendation']}。"
        ),
        (
            f"晚上想记一笔：{seed['today_trigger']}。"
            f"之前{_strip_leading_yiqian(seed['past_state'])}。"
            f"{seed['product_reason_clause']}，喝下来{seed['state_observation']}。"
        ...
        (
            ...
            f"以前{_strip_leading_yiqian(seed['past_state'])}，{seed['product_reason_clause']}。"
            f"喝了一段时间，{seed['state_observation']}。{seed['recommendation']}。"
        ),
    ]
    return random.choice(shapes)
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v454 self-protection-boundary-cleanup probe preview",
        "",
        ...
        "相对 v453，本版只清理自护力边界规则：保留“不写自护力”，删除规则里显式喂给模型的“保护力、状态稳、精神头”等替代表达。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证删除替代表达提示后，“精神头”高频是否下降。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v454 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v454`",
...
def _candidate_slots() -> list[str]:
    return [
        ...
        "state_observation：由 now_change + daily_scene 合并而来，仍是核心胚子内部的可替换描述，不作为末尾素材包",
