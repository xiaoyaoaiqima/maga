...
EXPERIMENT_ID = "v452_child_state_trigger_cleanup_probe"
...
def main() -> None:
    ...
    response = {
        ...
        "model": config.model,
        "core_chain": "short inline core brief: diary trigger + prior weak state + normalized Wangyue protection phrase + varied child-state observation + recommendation",
...
def _build_core_content(seed: dict[str, str]) -> str:
    shapes = [
        ...
        (
            ...
            f"旺玥这罐我主要看{seed['selling_expression_inline']}，喝下来{seed['state_observation']}。"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v452 child-state-trigger-cleanup probe preview",
        "",
        ...
        "相对 v451，本版继续清理孩子状态源头：去掉 today_trigger 里的“说个不停”强诱导，并扩充状态观察池，避免模型反复落到“叽叽喳喳/翻绘本/搭积木”。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证“孩子状态触发 + 状态观察”从源头扩散后，是否能降低同质化且不破坏种草主线。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v452 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v452`",
