...
EXPERIMENT_ID = "v447_inline_core_content_probe"
...
- 正文120-180字左右。
- 像妈妈顺手发，不要像广告brief。
- 核心内容里已经包含主线和卖点，写正文时只把它自然展开，不要再补新的选择理由或第二个生活入口。
"""
...
def main() -> None:
    ...
    response = {
        ...
        "model": config.model,
        "core_chain": "one inline paragraph: diary trigger + prior weak state + matched Wangyue protection expression + current state proof + recommendation",
...
def _build_core_content(seed: dict[str, str]) -> str:
    return (
        f"{seed['today_trigger']}，想来小红书记一下带娃日常。"
        f"{seed['past_state']}。"
        f"这段时间家里喝旺玥，3岁以上孩子喝的4段儿童奶粉，"
        f"{seed['selling_expression_reference']}，我主要把它放在日常保护力这块看。"
        f"喝下来，{seed['now_change']}，{seed['daily_scene']}。"
        "最后自然提一句，其他有类似困扰的妈妈也可以试试旺玥。"
    )
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v447 inline-core-content probe preview",
        "",
        ...
        "",
        "这批验证一个更小的核心：先把生活入口、过去状态、旺玥相关卖点、现在变化和推荐出口渲染成一整段核心内容，再交给模型扩写。",
        "本版不再单独列产品事实，也不把槽位 JSON 塞在末尾；槽位只在核心内容内部替换。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证“槽位嵌入核心内容内部”是否比末尾素材包更自然。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v447 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v447`",
...
def _candidate_slots() -> list[str]:
    return [
        ...
        "inline_core_content：本轮不是新槽位，而是把可替换描述直接嵌入核心内容段落",
