#!/usr/bin/env python3
"""Local Wangyue human-event/product-bridge experiment.

This does not replace the production content.generate path. It tests whether
separating a product-free human event from the product bridge improves realness.
"""
...
EXPERIMENT_ID = "v405_human_event_bridge"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v405 human-event product-bridge architecture experiment preview",
        "",
        ...
        "",
        "这一版验证架构假设：先生成不含产品的人类事件，再单独生成产品进入桥，避免 planner 直接写出顺滑广告链。",
        "本地粗审不是生产审核，重点看 human_event 本身是否成立，以及 product_bridge 是否把旺玥从因素写成答案。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(
    ...
    return "\n".join([
        "# v405 human-event product-bridge item1 rendered prompt",
        "",
