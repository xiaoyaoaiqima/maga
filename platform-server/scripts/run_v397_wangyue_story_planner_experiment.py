...
EXPERIMENT_ID = "v397_story_planner"
...
def _validate_plan(plan: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    if any(word in text for word in ("安心", "省心", "放心", "踏实", "心里有底")):
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v397 planner-writer architecture experiment preview",
        "",
        ...
        "",
        "这一版验证架构假设：先生成可审核主线，再写正文，是否比一次性把所有槽位塞给 writer 更能减少槽位拼接。",
        "本地粗审不是生产审核，重点看主线中间物是否让问题提前暴露。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(plan_prompt: str, writer_prompt: str, plan: dict[str, Any]) -> str:
    return "\n".join([
        "# v397 planner-writer item1 rendered prompt",
        "",
        "## Planner System",
        "",
        "```text",
