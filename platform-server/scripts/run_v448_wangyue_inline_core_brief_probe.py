...
EXPERIMENT_ID = "v448_inline_core_brief_probe"
...
def main() -> None:
    ...
    response = {
        ...
        "model": config.model,
        "core_chain": "short inline core brief: diary trigger + prior weak state + matched Wangyue protection expression + current state proof + recommendation",
...
def _build_core_content(seed: dict[str, str]) -> str:
    shapes = [
        (
            ...
            f"以前{_strip_leading_yiqian(seed['past_state'])}；这段时间喝旺玥，主要看中{seed['selling_expression_reference']}。"
            f"现在{seed['now_change']}，{seed['daily_scene']}。类似情况可以试试旺玥。"
        ),
        (
            f"晚上想记一笔：{seed['today_trigger']}。"
            f"之前{_strip_leading_yiqian(seed['past_state'])}。"
            f"旺玥这罐我主要看{seed['selling_expression_reference']}，喝下来{seed['now_change']}，{seed['daily_scene']}。"
            "有相似困扰的妈妈可以看看旺玥。"
        ),
        (
            f"{seed['today_trigger']}，才反应过来这阵子变化挺明显。"
            f"以前{_strip_leading_yiqian(seed['past_state'])}。"
            f"家里这段时间喝旺玥，{seed['selling_expression_reference']}。"
            f"现在{seed['daily_scene']}，整体{seed['now_change']}。可以推荐同阶段孩子试试旺玥。"
        ),
        (
            f"带娃日常随手记：{seed['today_trigger']}。"
            f"以前{_strip_leading_yiqian(seed['past_state'])}，所以旺玥我主要看{seed['selling_expression_reference']}。"
            f"喝了一段时间，{seed['now_change']}，{seed['daily_scene']}。类似情况可以试试旺玥。"
        ),
    ]
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v448 inline-core-brief probe preview",
        "",
        ...
        "",
        "这批验证一个更小的核心：先把生活入口、过去状态、旺玥相关卖点、现在变化和推荐出口渲染成短主线胚子，再交给模型扩写。",
        "相对 v447，本版不再给完整范文式核心内容，避免骨架被锁死。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证“槽位嵌入短核心内容胚子”是否兼顾自然度和多样性。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v448 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v448`",
