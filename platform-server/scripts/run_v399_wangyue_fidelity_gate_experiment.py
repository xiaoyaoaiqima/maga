#!/usr/bin/env python3
"""Local Wangyue story planner + fidelity gate experiment.

This does not replace the production content.generate path. It tests whether
writer output stays faithful to an approved story plan.
"""
...
EXPERIMENT_ID = "v399_fidelity_gate"
...
def main() -> None:
    ...
    for index, row in enumerate(selected, start=1):
        ...
        if plan_valid:
            ...
            quality["hard_pass"] = bool(quality["hard_pass"] and fidelity["pass"])
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v399 fidelity-gate architecture experiment preview",
        "",
        ...
        "",
        "这一版验证架构假设：先生成可审核主线，主线通过才写正文，正文写完后再检查 writer 是否背叛主线。",
        "本地粗审不是生产审核，重点看 fidelity gate 能否把固定喝法、广告标题、安心收口和额外效果证明显性化。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(plan_prompt: str, writer_prompt: str, plan: dict[str, Any]) -> str:
    return "\n".join([
        "# v399 fidelity-gate item1 rendered prompt",
        "",
        "## Planner System",
        "",
        "```text",
