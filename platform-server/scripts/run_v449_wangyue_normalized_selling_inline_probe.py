...
EXPERIMENT_ID = "v449_normalized_selling_inline_probe"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v449 normalized-selling-inline probe preview",
        "",
        ...
        "相对 v448，本版只修卖点表达入句前的清洗/归一化，避免完整广告句硬塞进妈妈口吻。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证“旧卖点表达归一化后嵌入短核心内容胚子”是否兼顾自然度和多样性。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v449 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v449`",
