#!/usr/bin/env python3
"""Local Wangyue plan gate calibration experiment.
...
EXPERIMENT_ID = "v402_plan_gate_calibration"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v402 plan-gate calibration architecture experiment preview",
        "",
        ...
        "",
        "这一版验证架构假设：校准 plan_gate，减少否定句误伤，同时把产品形态幻觉交给 surface/local quality。",
        "本地粗审不是生产审核，重点看 plan_rejected 是否更真实，产品形态风险是否显性化。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(plan_prompt: str, writer_prompt: str, plan: dict[str, Any]) -> str:
    return "\n".join([
        "# v402 plan-gate calibration item1 rendered prompt",
        "",
        "## Planner System",
        "",
        "```text",
