#!/usr/bin/env python3
"""Local Wangyue v441 lean product-relation prompt probe.

v440 proved the two-step relation planner is more promising than the old
three-step human-event bridge. v441 keeps the same architecture but reduces the
prompt surface:

1. planner receives only a lean source summary and the current target kernel.
2. writer receives only the approved relation plan and product hard facts.
"""
...
EXPERIMENT_ID = "v441_lean_relation_prompt_probe"
...
- 优先服从 target_kernel；如果 source_summary 明显不适配，可以在同类产品关系内微调。
- self_check 只写一句内部检查，不要复述禁词或规则原文。
"""
...
- 不要新增 approved_relation_plan 没有的固定喝法、频率、长期习惯或第二个证明。
"""
...
def _build_plan_prompt(row: dict[str, Any], index: int, kernel: dict[str, str]) -> str:
    payload = {
        ...
        "source_summary": _lean_source_summary(row, index),
        "target_kernel": kernel,
        "task": "基于 source_summary 和 target_kernel，规划一条产品关系阶段清楚、证明机制单一的旺玥UGC种草主线。",
    }
    return json.dumps(payload, ensure_ascii=False, indent=2)


def _build_writer_prompt(row: dict[str, Any], index: int, kernel: dict[str, str], plan: dict[str, Any]) -> str:
    clean_plan = {key: value for key, value in plan.items() if key != "self_check"}
    payload = {
        "item_index": index,
        "product_fact": "旺玥是3岁以上、3-6岁学龄前儿童可喝的4段儿童奶粉。",
        "approved_relation_plan": clean_plan,
        "writer_task": "写一篇小红书妈妈UGC正向种草笔记，只沿 approved_relation_plan 的一条主线写。",
    }
    return json.dumps(payload, ensure_ascii=False, indent=2)


def _lean_source_summary(row: dict[str, Any], index: int) -> dict[str, Any]:
    return {
        "source_row_no": row.get("source_row_no") or row.get("item_no") or index,
        "post_type": row.get("post_type"),
        "painpoint": row.get("painpoint"),
        "selling_point": row.get("selling_point"),
        "selling_hint": _shorten(row.get("selling_description"), 80),
        "story_hint": _shorten(row.get("story_spine"), 90),
        "boundary_hint": _shorten(row.get("corpus"), 70),
    }
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v441 lean product-relation prompt probe preview",
        "",
        ...
        "",
        "这一版验证 v440 的两段式关系规划能否进一步瘦 prompt：planner 只看极简 source 摘要，writer 只看 approved_relation_plan。",
        "重点看旧语料重复强化是否下降，尤其是固定喝法、频率证明和 source_row 复读。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(index: int, row: dict[str, Any], kernel: dict[str, str], plan_prompt: str, plan: dict[str, Any], writer_prompt: str) -> str:
    return "\n".join([
        "# v441 lean product-relation rendered prompt",
        "",
        f"- item_no: `{index}`",
