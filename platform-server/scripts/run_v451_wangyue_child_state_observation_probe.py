...
EXPERIMENT_ID = "v451_child_state_observation_probe"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v451 child-state-observation probe preview",
        "",
        ...
        "相对 v450，本版把 now_change 和 daily_scene 合并为一个状态观察胚子，降低“叽叽喳喳/翻绘本/搭积木”这类孩子状态表达的重复。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证“孩子状态观察”在核心链路内部扩散后，是否能降低同质化且不破坏种草主线。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v451 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v451`",
