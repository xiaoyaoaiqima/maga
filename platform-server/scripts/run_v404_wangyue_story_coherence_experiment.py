#!/usr/bin/env python3
"""Local Wangyue storyline coherence experiment.

This does not replace the production content.generate path. It tests whether
an explicit posting impulse and storyline can keep the writer from stacking
independent slots into one article.
"""
...
EXPERIMENT_ID = "v404_story_coherence"
...
PLANNER_SYSTEM = """你是小红书母婴UGC内容的发帖动机规划器。
你不写正文，只规划“这个妈妈为什么会发这条”。
目标是：先有一个真实发帖冲动，再让旺玥在这条生活主线里出现；不要把痛点、卖点、生活细节和效果证明并列摆成素材清单。
输出 JSON，字段只能是：
posting_motive, storyline, life_entry, product_permission, product_role, single_selling_point, positive_evidence, ending_stop, avoid_links, self_check。
要求：
- posting_motive 写“为什么想发”，storyline 写“注意力如何从生活现场移动到旺玥，再停在哪里”。
- 只允许一条主线，一个生活触发，一个产品判断，一个自家观察；其他素材都必须服从 storyline。
...
def _build_writer_prompt(row: dict[str, Any], plan: dict[str, Any], *, plan_valid: bool, plan_issues: list[str]) -> str:
    payload = {
        ...
        "storyline_contract": "approved_story_plan.storyline 是主线，source_row 只提供可用事实；如果事实素材和主线不顺，优先保主线，不要把素材并列拼上去。",
...
def _local_quality(title: str, body: str, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(word in text for word in ("奶瓶", "自己泡", "书包侧袋", "水杯侧袋", "便携", "一包", "小包", "条装", "随身带")):
...
def _surface_guard(title: str, body: str) -> dict[str, Any]:
    ...
    if any(term in body for term in ("一包", "小包", "条装", "便携装", "随身带")):
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v404 story coherence architecture experiment preview",
        "",
        ...
        "",
        "这一版验证架构假设：先规划“妈妈为什么发这条”和一条 storyline，再要求 writer 沿这条线写正文。",
        "本地粗审不是生产审核，重点看正文是否仍然出现槽位并列拼接、产品突然跳入、结尾断线。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(plan_prompt: str, writer_prompt: str, plan: dict[str, Any]) -> str:
    return "\n".join([
        "# v404 story coherence item1 rendered prompt",
        "",
        "## Planner System",
        "",
        "```text",
