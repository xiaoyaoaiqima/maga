...
EXPERIMENT_ID = "v444_clean_selling_expression_probe"
...
- 不写当前季节或当前疾病大环境。
- 不要写成医生建议、治疗、保证有效。
- 不用“可能、也许、不知道是不是这个奶粉的功劳”这类免责声明式表达；要用妈妈大白话正面说喝下来看到的变化。
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v444 clean-selling-expression probe preview",
        "",
        ...
        "本版只接回已有卖点表达槽位，不新增 Product Reason；相对 v443，先过滤固定杯数、年龄外溢、价格、护盾类广告比喻和强KOL词。",
        "",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v444 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v444`",
