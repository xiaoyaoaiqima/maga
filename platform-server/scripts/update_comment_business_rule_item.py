...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--asset-type", default="comment_business_rule_set")
    ...
    selector.add_argument("--business-rule", default=None)
    ...
    parser.add_argument("--created-by", default="comment-business-rule-item-tool")
    return parser
...
def _content_with_updated_corpus(
    ...
    *,
    ...
    summary = {
        ...
        "old_business_rule": old_business_rule,
...
def _extract_examples_from_corpus(corpus: str) -> list[str]:
    """Extract operator examples from the standard comment business-rule corpus block."""
    examples: list[str] = []
    in_examples = False
...
def _find_item(items: list[Any], args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    for index, item in enumerate(items):
        ...
        if args.rule_id is not None and str(item.get("rule_id") or "") == str(args.rule_id):
            ...
            matches.append((index, item))
        elif args.business_rule is not None and str(_business_rule_name(item) or "") == str(args.business_rule):
            matches.append((index, item))
    if not matches:
        raise ValueError("no matching comment business-rule item found")
    if len(matches) > 1:
        candidates = [
            {
                "index": index,
