...
EXPERIMENT_ID = "v453_product_reason_phrase_probe"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v453 product-reason-phrase probe preview",
        "",
        ...
        "相对 v452，本版继续清理产品理由句式源头：把“主要看/主要看中”改成多种妈妈能说出口的产品理由连接句。",
        "",
        "## Scope Note",
        "",
        "本轮不新增 Product Reason，也不正式抽多个槽位；只验证“卖点短语如何接进核心链路”是否能降低同质化且不破坏种草主线。",
...
def _format_prompt_sample(item: dict[str, Any]) -> str:
    return "\n".join([
        "# v453 item1 rendered generation prompt",
        "",
        f"- batch_id: `local_v453`",
