#!/usr/bin/env python3
"""Local Wangyue story planner + surface guard experiment.

This does not replace the production content.generate path. It tests whether
writer output stays faithful to an approved story plan and keeps surface hygiene.
"""
...
EXPERIMENT_ID = "v400_surface_guard"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v400 surface-guard architecture experiment preview",
        "",
        ...
        "",
        "这一版验证架构假设：把表面卫生从 fidelity gate 拆出来。标题禁用 emoji/禁词可局部清理；正文禁词直接 surface fail。",
        "本地粗审不是生产审核，重点看失败归因是否更清楚：plan、fidelity、surface 各归各位。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(plan_prompt: str, writer_prompt: str, plan: dict[str, Any]) -> str:
    return "\n".join([
        "# v400 surface-guard item1 rendered prompt",
        "",
        "## Planner System",
        "",
        "```text",
