#!/usr/bin/env python3
"""Local Wangyue product-eligible human-event experiment.

This does not replace the production content.generate path. It tests whether
product-eligible human events improve product entry without business-row leakage.
"""
...
EXPERIMENT_ID = "v407_product_eligible_event"
...
def _write_preview(path: Path, response_path: Path, prompt_path: Path, plan_csv_path: Path, response: dict[str, Any]) -> None:
    ...
    lines = [
        "# v407 product-eligible human-event architecture experiment preview",
        "",
        ...
        "",
        "这一版验证架构假设：human_event 只吃产品有进入资格的生活主题，但仍不吃旺玥卖点；product_bridge 才接收旺玥信息。",
        "本地粗审不是生产审核，重点看产品进入资格是否比 v406 提升，同时不要回到 v405 的业务 row 污染。",
        "",
        "## Items",
        "",
    ]
...
def _format_prompt_sample(
    ...
    return "\n".join([
        "# v407 product-eligible human-event sampled rendered prompt",
        "",
