#!/usr/bin/env python3
"""
Batch prompt optimizer MVP with a shared prompt file.

Reads one fixed generation prompt from a TXT file, reads title/content/problem
samples from a CSV, calls an OpenAI-compatible chat completions API, and writes
prompt diagnosis results to a CSV.
"""
...
REQUIRED_COLUMNS = ("title", "content", "problem")
OUTPUT_COLUMNS = (
    "id",
    "prompt_file",
    "prompt_issue",
...
你需要根据一份固定的原始生文提示词、多篇生成结果和运营审查问题，反推出提示词设计上的缺陷，并给出可执行的修改方案。

请注意：CSV 中每一行文章的差异主要来自多样性生成，生文提示词本身是同一份。因此你要重点判断固定提示词中哪些不变部分导致了当前样本的问题，而不是把问题简单归因于单篇文章偶然发挥。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Read one prompt TXT plus a CSV and batch optimize the shared generation prompt.",
    )
    parser.add_argument("--prompt-file", required=True, help="TXT file containing the shared generation prompt.")
    parser.add_argument("--input", required=True, help="Input CSV path. Required columns: title, content, problem.")
    parser.add_argument("--output", required=True, help="Output CSV path.")
    parser.add_argument("--id-column", default="id", help="Optional ID column name. Defaults to id.")
...
def ensure_output(path: Path, input_columns: list[str]) -> list[str]:
...
def build_user_prompt(row: dict[str, str], shared_prompt: str) -> str:
...
async def optimize_one(row: dict[str, str], shared_prompt: str, args: argparse.Namespace) -> dict[str, str]:
    user_prompt = build_user_prompt(row, shared_prompt)
    raw_output = ""
    last_error = ""
...
async def run_batch(args: argparse.Namespace) -> None:
    ...
    shared_prompt = read_prompt_file(prompt_path)
    ...
    output_columns = ensure_output(output_path, input_columns)
    ...
    async def worker(index: int, row: dict[str, str]) -> None:
        async with semaphore:
            ...
            result = await optimize_one(row, shared_prompt, args)
