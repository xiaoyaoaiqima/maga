...
Reads content/critic_prompt/critic_result/human_feedback from four TXT files,
...
你需要根据被审核内容、原始审核规则提示词、模型审核结果和人工反馈，反推出审核提示词的问题，并给出可直接执行的修改方案。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Read critic prompt debug TXT files and optimize the critic prompt.",
    )
    parser.add_argument("--content-file", required=True, help="TXT file containing audited content.")
    parser.add_argument("--critic-prompt-file", required=True, help="TXT file containing the critic prompt.")
    parser.add_argument("--critic-result-file", required=True, help="TXT file containing the model critic result.")
    parser.add_argument("--human-feedback-file", required=True, help="TXT file containing human feedback.")
    ...
    *,
    ...
    payload = {
        ...
        "input_files": {
            ...
            "critic_prompt_file": str(Path(args.critic_prompt_file)),
            "critic_result_file": str(Path(args.critic_result_file)),
            "human_feedback_file": str(Path(args.human_feedback_file)),
        },
        "model_params": {
...
def main() -> int:
    ...
    try:
        ...
        critic_prompt = read_required_text(Path(args.critic_prompt_file), "critic_prompt")
        critic_result = read_required_text(Path(args.critic_result_file), "critic_result")
        human_feedback = read_required_text(Path(args.human_feedback_file), "human_feedback")
