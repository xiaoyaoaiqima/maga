...
WORKER_WORKSPACE="${WORKER_WORKSPACE:-/Users/luxifa/.hermes/profiles/maga-worker/workspace}"
...
start_worker() {
  mkdir -p "$(dirname "$WORKER_LOG")" "$(dirname "$WORKER_PID")"
  : >"$WORKER_LOG"
  load_env
  ...
  "$PYTHON_BIN" - "$ROOT_DIR" "$WORKER_WORKSPACE" "$WORKER_LOG" "$WORKER_PID" "$WORKER_HOST" "$WORKER_PORT" <<'PY'
import os
import pathlib
import subprocess
import sys

root_dir, worker_workspace, worker_log, worker_pid, worker_host, worker_port = sys.argv[1:7]
env = os.environ.copy()
env.update(
    {
