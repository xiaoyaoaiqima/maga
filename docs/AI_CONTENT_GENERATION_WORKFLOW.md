...
a2礼遇默认按老客分享处理。认可表达根据本篇抽中的原始素材，自然承接活动、每批检测、长期购买背景或实际使用感受；生成要求不再强制单一活动了解途径，也不再用“信息了解后认可 / 老客使用感受”限制正文能否出现老客背景或产品体验。活动奖品只介绍可兑换、可领取或可抽取什么；标题和正文都不得写成自己已经领了、领到、收到、拿到、兑到、换到或中奖。“囤了好几罐”不写进生文 Prompt，作为业务硬 ban 交给机器审核。标题生成时要求少于19字；审核允许20字，只有超过20字才直接淘汰。
