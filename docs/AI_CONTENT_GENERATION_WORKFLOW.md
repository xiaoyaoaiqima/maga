...
- 莼悦旧 RAAP 导出中 `人设+沟通策略+卖点` 的原始卖点表达已经业务确认：原文里的强因果、成分和产品事实都可以直接用于生文，不按通用风险话术主动降调。确定性审核只拦模型在本篇原文之外新增的认证、成分、数字、周期、作用机制或效果；不能因为原文本身力度强就判错或改弱。
- 莼悦运行时除品牌名和“家里正在喝/正在选莼悦”这类中性关系外，产品事实、孩子变化和效果只准使用本篇卖点痛点原文。原文中的各段内容必须按原顺序逐字保留，允许拆开穿插渠道、时间、地点、动作和心情；不得改写原文片段，也不得新增产品事实、孩子变化、效果、前后对比或因果。痛点只作为妈妈关注或选奶的动机，不要求模型另写一个改善结果。机器验收按标点拆分原文，检查非空片段是否逐字且依次出现在正文中；片段之间插入渠道或生活场景不判失败。
- 莼悦不得把本篇卖点原文写成罐身、包装、标签、说明或配料表上的文字。信息来源不再由全局生成要求枚举，而是在 `生文素材` 的 `【信息来源素材】` 槽位中按篇抽取：可以给具体渠道短标签、给抽象来源，也可以抽中“正文不写来源”。来源选项只写点位或短动作，不写成半成品句子；Prompt 不再额外解释来源槽的使用规则。
...
- 上一条只定义审核如何判断，不代表要在生文 Prompt 中主动提醒模型写感冒、请假、全勤或同伴生病；没有本篇业务需要时，不渲染这类提示，避免诱发额外疾病场景。
...
旺玥 V3 不再调用旧的产品体验超级审核 Prompt。自动后处理固定为：

```text
代码硬审
→ Temporal Logic / Claim-Public Disease / Content Fit / Fluency
→ 程序聚合 pass / watch / block
→ block 按问题类型局部改写
→ Rewrite Quality Validator + Focused Judges 复审
→ 决定入池或 hold
```

- `watch` 保留原文，不自动改写，可以正常入池。
- `block` 只有在局部改写、代码硬审、改写质量验收和完整 Focused 复审均通过后才写回正文。
- 任一专用 Judge 路由不可用时进入 hold，不回退到文章生成模型或旧超级审核 Prompt。
- 代码硬审失败后不允许 LLM 改写把文章重新放行。
- Temporal Logic、Claim/Public Disease、Content Fit 使用 `deepseek-v4-flash`；Fluency 和 Rewrite Quality 使用 `qwen-plus`。
- 其他业务暂时继续使用原产品体验审核链路。
