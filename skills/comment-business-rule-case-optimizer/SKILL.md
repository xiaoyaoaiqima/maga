---
name: comment-business-rule-case-optimizer
description: 按单个评论业务规则子方向逐条优化语料示例、吸收运营反馈、写回指定 MAGA asset 并跑小批量验证；适用于用户说“by case 看评论业务规则”“一个一个改示例”“改完用这个业务规则跑几条看看”。
---

# 评论业务规则 by case 优化

用于把运营现场反馈沉淀到某一个评论业务规则子方向里，再用小批量生文验证。调用时必须明确当前任务使用的 `asset_key`；不要在 skill 内硬编码具体活动资产。默认不要加重入口生成要求，优先改当前子方向的示例池。

## 工作流

1. 明确当前任务参数。
   - 必须从用户上下文、任务配置或本轮调查中确认 `asset_key`。
   - 必须确认定位方式：`rule_id`、`source_row_no` 或 `business_rule` 三选一。
   - 不确定当前资产时，先查询/询问，不要凭历史记忆使用某个活动资产。
2. 读取当前激活 asset 的真实子方向，不凭记忆改。
   - 查看单条：`PYTHONPATH=. ../.venv/bin/python scripts/update_business_rule_rule_item.py --asset-key <asset_key> --rule-id <rule_id> --show-current`
   - 也可用 `--source-row-no <row_no>` 或 `--comment-business-rule <name>` 定位。
3. 和用户逐条确认 case。
...
6. 写回时同步 `corpus` 和 `examples`。
   ...
   - dry-run：`PYTHONPATH=. ../.venv/bin/python scripts/update_business_rule_rule_item.py --asset-key <asset_key> --rule-id <rule_id> --corpus-file <file> --sync-examples-from-corpus`
   - 发布：同命令加 `--apply`，默认使用 `new-version`。
7. 验证时只跑当前业务规则。
   - 优先调用 `POST /api/v1/content-agent/comment-batches/start`，传 `asset_key` + `rule_id` 或 `source_row_no` + `count`，精准重复当前子业务规则。
   - 只有在无法定位 `rule_id/source_row_no` 时，才退回用 `business_rule` 过滤；此时要提醒用户同名/同类业务规则可能混在一起。
   - 跑完在回复里贴正文和问题摘要；如链路会产出文件，再给出 batch id 和文件路径。

## 精准试跑接口

后端已有单个子业务规则试跑能力，by case 优化时优先使用这个接口：
...
```json
{
  "asset_key": "<asset_key>",
  "rule_id": "<rule_id>",
  "draft_corpus": "这里放当前修改中的评论业务规则语料块",
  "count": 10,
  "created_by": "ops"
}
```

前端如果只传 `business_rule`，只能按大业务规则名称过滤；当一个大业务规则下有多个子方向时，会混跑多个子方向。更顺的运营入口是：
...
- 通用 prompt、系统关键词和守卫只做基础边界；业务信息优先沉淀到当前评论业务规则的规则和示例。
